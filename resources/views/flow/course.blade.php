<div id="courseDetails" data-course-url="{{url('/ajax/courseedit')}}" data-course-id="{{$course->id}}">
    ...
    <aside class="actions">
        ...
        @can(Permissions::COURSE_DELETE)
            <a href="{{url('/courses/' . $course->id . '/delete')}}"
                onclick="return actionVerify(event,'{{'delete '.$course->name}}');" class="delete">Delete</a>
        @endcan
    ...
    @if($role->hasPermissionTo(Permissions::COURSE_EDIT))
        <button class="toggleEditMode" onclick="toggleEditMode(this);" tooltip="ToggleEditMode"></button>
    @endif
        <button class="expandAll" onclick="expandModules();" tooltip="Expand All Modules"></button>
        <button class="collapseAll" onclick="collapseModules();" tooltip="Collapse All Modules"></button>
        </aside>
