...
<li id="project_{{$project->id}}" class="project component {{$css_class}}">
    @if($role->hasPermissionTo(Permissions::PROJECT_EDIT))
        <div class="actions">
            <button class="edit" data-item-type="project" data-item-id="{{$project->id}}">Edit</button>
            @if($project->teams_enabled)
                <button class="teams" onclick="displayTeamsList({{$project->id}})" tooltip="Show teams for this project">Show Teams</button>
            @endif
            </div>
    ...
    <a href="{{url('/code/project/' . $project->id)}}">
        ...
        <span class="teams{{$project->teams_enabled ? " enabled": " disabled"}}">
            {{$project->teams_enabled ? "Enabled": "Disabled"}}
        </span>
